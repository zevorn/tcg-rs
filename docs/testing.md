...
逐层验证正确性。所有测试集中在独立的 `tests/` crate 中（少量
集成测试除外），保持源码文件干净，同时验证公共 API 的完整性。
...
cargo test -p tcg-linux-user    # linux-user 集成测试
...
| linux-user | 6 | 0.7% | ELF 加载、客户程序执行 |
...
**自动化集成测试**（`linux-user/tests/guest_exec.rs`）：
...
二进制执行 hello ELF，验证输出为 `Hello, World!\n`。如果
交叉编译器未安装，测试会自动跳过。
